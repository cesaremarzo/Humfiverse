...
A third contract handles peer-to-peer resale between token holders: a seller lists from their own wallet, and a purchase moves the tokens and the payment in one transaction, with the platform's 1% fee retained by the contract. Whether resale is permissible for a real offering is still an open compliance question (see [Legal & Regulatory Structure](05-legal-structure.md)). The live contracts pass 79 automated tests. That's real test coverage, not a professional security audit — the two aren't the same thing (see [Risk Factors](08-risk-factors.md)).
