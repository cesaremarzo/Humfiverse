...
| Storico dei versamenti di royalty on-chain (`royalty_deposits`): hash della transazione, id del token, **indirizzo wallet di chi versa**, importo, riferimento al rendiconto (hash) e, se chi versa lo comunica, il testo del rendiconto; blocco, data e ora (§2.95) | Blockchain pubblica; il testo del rendiconto da chi versa, accettato solo se il suo hash coincide con quello scritto on-chain | Storico dei versamenti nella pagina dell'asset | Esecuzione del servizio | [Finché l'asset è pubblicato]; tutto tranne il testo del rendiconto è copia di dati pubblici on-chain. Il testo è già pubblico nella pagina: chi versa non deve inserirvi dati personali di terzi |
