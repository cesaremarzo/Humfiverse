...
| Commit rivisto | §2.100 (guida automatica: nuova tabella `assistant_requests`, Anthropic come fornitore). `fd7f822` (§2.98: rendiconto royalty come file, chi versa, intervallo per token). Prima il redeploy di fase 2 su `dev/cesare` (technical §2.92, §2.95-§2.97): royalty on-chain, rimborsi per token con burn, motivi di annullamento, operator, proprietà a un Safe 2 su 3. Prima `94ae18f` (§2.94) |
