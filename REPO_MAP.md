...
- `assistant-knowledge.js` — everything the guide widget's AI mode is allowed to say (§2.100): the platform brief and the rules it answers under. Keep it in step with the product; `legal/check.sh` watches it.
...
- `webapp/src/app/core/` — the non-visual layer:
  ...
  - `assistant-kb.ts` / `assistant.service.ts` — the guide widget's twelve written topics and the state behind the panel (§2.100). The topics are the whole of its guided mode and the fallback whenever the AI mode cannot answer.
...
- `webapp/src/app/layout/` — top-level app shell (nav, topbar, the sign-in dialog, the guide widget bottom-right).
- `webapp/public/assets/art/` — rendered decorative images (vinyl, coins, microphone, VU meter, film grain) used by the backdrop and the landing; generated by the Python scripts in `webapp/art-src/` (§2.99), not hand-drawn — re-render rather than edit.
